...
public class GestionDesSeries {
	...
	private static void rechercherSerie() {
		...
		System.out.println("l'étudiant est dans la série : " + classe.numeroSerie(nom));
	...
	private static void afficherTailleSeries() {
		for (int i = 0; i < classe.nombreSeries(); i++) {
			int serie=i+1;
			System.out.println("Dans la série "+ serie +" il ya : " + classe.getSerieEtudiants(serie).nombreEtudiants()+ " étudiant");
		}
