...
public class Troupe {
	...
	public boolean ajouterScout(String scout, String patrouille){
		...
		if(patrouille == null || patrouille.length()==0)
			throw new IllegalArgumentException();
		if(ensembleScouts.contains(scout))return false;

		Noeud noeud=new Noeud(scout);
		Noeud cp=mapPatrouilleNoeudCP.get(patrouille);
		if(cp==null)return false;
		noeud.suivant=cp.suivant;
		cp.suivant=noeud;
	...
	public boolean supprimerScout(String scout){
		if(scout == null || scout.length()==0)
			throw new IllegalArgumentException();
		if(ensembleCP.contains(scout) ) return false;
		if(!ensembleScouts.contains(scout)) return false;
	...
	public String donnerPatrouille(String patrouille){
		...
		String str=noeud.scout;
		noeud=noeud.suivant;

		while (noeud!=null){
			if(ensembleCP.contains(noeud.scout)) return str;
			str += " "+noeud.scout;
			noeud=noeud.suivant;
		}
		return str;
