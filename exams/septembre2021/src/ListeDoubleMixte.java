...
public class ListeDoubleMixte {
	...
	public int nombreNationalitesDifferentes(){
		HashSet<String> natio=new HashSet<>();
		for (JoueurTennis j: mapJoueurNoeud.keySet()) {
			natio.add(j.getNationalite());
		}
	...
	public boolean supprimerCouple(JoueurTennis joueur) {
		...
		Noeud noeudJ = mapJoueurNoeud.get(joueur);
		if (noeudJ == null) return false;
		JoueurTennis j;
		if(joueur.getSexe()=='H') {

			j= noeudJ.suivant.joueur;

			noeudJ.suivant.suivant.precedent = noeudJ.precedent;
			noeudJ.precedent.suivant = noeudJ.suivant.suivant;

		}
		else{
			j = noeudJ.precedent.joueur;

			noeudJ.suivant.precedent = noeudJ.precedent.precedent;
			noeudJ.precedent.precedent.suivant = noeudJ.suivant;

		}

		mapJoueurNoeud.remove(joueur);
		mapJoueurNoeud.remove(j);
