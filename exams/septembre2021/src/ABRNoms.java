...
public class ABRNoms implements Iterable<String>{
	...
	public int nombreNomsCommencantPar(char lettre){
		if(lettre>'z' || lettre<'a') throw new IllegalArgumentException();
	...
	private int nombreNomsCommencantPar(char lettre, NoeudNom noeud){
		...
		if(noeud.nom.charAt(0)==lettre) return 1+nombreNomsCommencantPar(lettre,noeud.gauche)+ nombreNomsCommencantPar(lettre,noeud.droit);
		return nombreNomsCommencantPar(lettre,noeud.gauche) + nombreNomsCommencantPar(lettre,noeud.droit);
	}
	...
	public HashSet<String> ensembleHomonymes(){
		...
		ensembleHomonymesRec(racine, tt, homonymes);
	...
	private void ensembleHomonymesRec(NoeudNom noeud, HashSet<String> tt, HashSet<String> homonymes) {
		if (noeud == null) {
			return;
		}

		if (tt.contains(noeud.nom)) {
			homonymes.add(noeud.nom);
		}else{
			tt.add(noeud.nom);
		}

		ensembleHomonymesRec(noeud.gauche, tt, homonymes);
		ensembleHomonymesRec(noeud.droit, tt, homonymes);
	}
